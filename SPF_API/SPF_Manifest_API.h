 ...
 *
 * void SPF_MapOrigin_BuildManifest(SPF_Manifest_Builder_Handle* h, const SPF_Manifest_Builder_API* api) {
 *
 *     // --- 1. Basic Identity ---
 ...
 *     out_api->BuildManifest = SPF_MapOrigin_BuildManifest;
 *     return true;
 * }
 * @endcode
 */
...
typedef struct SPF_Manifest_Builder_API {
     ...
     * @param name The unique name (e.g., "SPF_MapOrigin"). MUST match the name used in `GetContext` calls.
     ...
     *                   Example: "Movement" becomes "SPF_MapOrigin.Movement".
